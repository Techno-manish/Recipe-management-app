...
const getAllRecipes = asyncHandler(async (_, resp) => {
  try {
    ...
    return resp
      .status(200)
      .json(new ApiResponse(200, recipe, `All recipes fetched successfully`));
  } catch (error) {
...
const createRecipe = asyncHandler(async (req, resp) => {
  ...
  try {
    ...
    return resp
      .status(200)
      .json(new ApiResponse(200, response, `Recipe created successfully`));
  } catch (error) {
...
const getIdsOfSavedRecipes = asyncHandler(async (req, resp) => {
  try {
    ...
    resp
      .status(201)
      .json(new ApiResponse(200, user, `Recipe saved successfully`));
  } catch (error) {
...
const getSavedRecipe = asyncHandler(async (req, resp) => {
  try {
    ...
    resp
      .status(201)
      .json(
        new ApiResponse(200, savedRecipes, `saved Recipes fetch successfully`)
      );
  } catch (error) {
...
const getUserRecipes = asyncHandler(async (req, resp) => {
  ...
  try {
    ...
    return resp
      .status(200)
      .json(new ApiResponse(200, userRecipes, `recipes fetch successfully`));
  } catch (error) {
...
const updateUserRecipe = asyncHandler(async (req, resp) => {
  ...
  const updatedRecipe = await Recipe.findByIdAndUpdate(recipe, updateFields, {
    new: true,
    runValidators: true,
  });
...
const removeSaveRecipe = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    userId,
    { $pull: { savedRecipes: recipeId } }, // Removes the specified recipe from the savedRecipes array
